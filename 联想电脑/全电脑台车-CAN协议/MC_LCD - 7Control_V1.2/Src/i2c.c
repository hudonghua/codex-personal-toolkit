...
volatile uint8_t I2CMasterBuffer[IICBUFSIZE];
...
volatile uint32_t WrIndex = 1;

unsigned long  gIICErrS = 0;		// Start Err
unsigned long  gIICErrE = 0;		// Stop Err

unsigned char  gIICStat[32];
unsigned short  gIICSLen = 0;
/* 
...
//gIICStat[gIICSLen++] = StatValue;
  switch ( StatValue )
  {
	...
	if ( I2CCmd == RD_BIT )
	{
	  I2C0->I2DAT = I2CMasterBuffer[0] | RD_BIT;
	}
	I2C0->I2CONCLR = (I2CONCLR_SIC | I2CONCLR_STAC);
	...
	if ( I2CMasterState == I2C_STARTED )
	{
	  ...
	  I2C0->I2DAT = I2CMasterBuffer[WrIndex];
	  WrIndex++;
	...
	{   
	  ...
	  I2C0->I2DAT = I2CMasterBuffer[WrIndex]; /* this should be the last one */
	  WrIndex++;
	}
	else
	{
	...
	if ( 1 == I2CReadLength )
		I2C0->I2CONCLR = I2CONSET_AA;	/* assert ACK after data is received */
	else
...
//	I2CMasterBuffer[3+RdIndex] = LPC_I2C0->I2DAT;
	I2CSlaveBuffer[RdIndex] = I2C0->I2DAT;
	RdIndex++;
	...
	else if ( RdIndex == I2CReadLength-1 )
 	{
		I2CMasterState = DATA_ACK;
		I2C0->I2CONCLR = I2CONSET_AA;	/* assert ACK after data is received */
	}
...
** 
*****************************************************************************/
uint32_t I2CInit( uint32_t vI2cMode ) 
{
  SC->PCONP |= (1 << 7);	  // IIC0
  //SC->PCONP |= (1 << 19);	  // IIC1
  ...
  if ( vI2cMode == I2CSLAVE )
  {
	;//I2C0->I2ADR0 = AT24_ADDR;
  }    
...
{
	unsigned long iicDly = 0x00fffff;
