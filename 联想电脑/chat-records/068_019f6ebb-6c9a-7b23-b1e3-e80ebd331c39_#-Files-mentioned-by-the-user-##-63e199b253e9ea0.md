...
- Local source: `C:\Users\t250c\.codex\sessions\2026\07\17\rollout-2026-07-17T14-20-07-019f6ebb-6c9a-7b23-b1e3-e80ebd331c39.jsonl`

## Transcript

### developer 2026-07-17T06:20:12.680Z
...
### assistant 2026-07-17T06:20:12.681Z
...
### developer 2026-07-17T06:20:12.681Z
...
### assistant 2026-07-17T06:20:18.024Z

```text
我先只读盘点“7月17改”及相邻版本，再把近期源码差异聚焦到双回转的采集、计算和界面显示链路；不会修改文件。
```

### assistant 2026-07-17T06:21:16.630Z

```text
相邻目录没有独立的“上一版”文件夹，但工程内部保留了 7 月 1 日源码备份，足够做前后对照。我已看到两个高相关变化：编码器角度换算新增了左右臂分支，界面变量也交换了地址并叠加了车身角；正在逐行确认哪一个会造成“静止时跳数”。
```

### assistant 2026-07-17T06:23:23.999Z

```text
为确认“跳”是数值计算问题还是屏幕控件的有符号/倍率设置问题，我会按 ViewTech 工程解析流程只读核对 `0xE10/0xE11` 控件定义；这一步不会改 `.vt` 文件。
```

### assistant 2026-07-17T06:25:25.591Z

```text
屏幕工程已核实：Page 1 的 `0xE10/0xE11` 都是 1 位小数，固件乘 10 的倍率是对的；跳变并非小数位配置错误。当前最值得验证的是新增车身角的有符号解释，以及编码器跨零环绕。
```

### assistant 2026-07-17T06:30:25.350Z

```text
视频同伴已确认画面是“凿岩参数设置2”，因此我撤回把车身角当作本次现象根因的判断。现在改查这 7 个参数的地址、数组索引和近期页面逻辑；这类“整组一起乱跳”更像参数索引越界或读写地址错位。
```

### assistant 2026-07-17T06:34:32.886Z

```text
我继续把“页面 16 的数据源、保存读取流程、以及是否有数组越界”三条线对齐，重点确认这是数值本身变化，还是同一批 HMI 地址被别的页面覆盖。全程只读。
```

### assistant 2026-07-17T06:39:43.294Z

```text
视频与代码已经对上：每次“正向选择/自动连发”只是在移动光标，代码并没有改参数，却会触发一次整组重发；屏幕恰好在每次重发后换成新的一组值。现在我在核查串口回包缓冲是否会在这批连续写入时越界——这比“参数计算错了”更符合现象。
```

### assistant 2026-07-17T06:41:42.908Z

```text
我已经排除“页面本身改了参数”和简单串口接收缓冲越界；现在把 7 月 17 日新增代码与 7 月 1 日备份逐项对比，重点找会覆盖这组参数内存的数组越界。全程只读，不改工程。
```

### assistant 2026-07-17T06:49:31.845Z

```text
已经找到能把现象完整串起来的硬证据：工程只给主栈分了 768 字节，但当前链接报告明确计算出最大栈深 1672 字节。栈会向下越过堆和若干全局变量，实际压进 `gPwm_BD` 参数区；页面每次移动光标再重发参数时，就把被栈覆盖后的新值显示出来，因此整组“跳”。我正在核对精确覆盖地址和旧版栈占用，确认这是不是本次新增计算链带来的回归。
```

### assistant 2026-07-17T06:53:45.206Z

```text
我继续按“已确认 / 依赖协议或机械方向假设”分开审计；现在先核对 `App_lcd.c` 新增字符串构造函数的目标缓冲区大小，再扫 `App_usr.c` 的近期控制改动。全程只读，不改文件。
```

### assistant 2026-07-17T06:56:42.551Z

```text
只读对比完成，未修改文件。除栈溢出外：

- 已确认近期问题：
  - `App_usr.c:4102-4107` 重复清零 `ZY_arm_L`，漏清 `ZY_arm_R`，会残留上一周期输出。将 4106 改为 `ZY_arm_R=0`。
  - `App_usr.c:2535-2583` 的 `0x000` 帧只填写 `CAN_SBuf2[0..1]`，其余 6 字节沿用共享发送缓冲旧值。发送前清零 8 字节或改用专用缓冲。
  - `App_usr.c:5711-5735` 删除了旧版 NO1 四路 `BoomRampLimit()`，而 NO3 在 `5754-5772` 仍有限幅保护。建议恢复 NO1 四路保护。

- 需现场方向或协议确认：
  - `3952-3960` 的 NO3 上下符号相对旧版反转，`4007-4013` 的 `ZY_arm_L/R` 对应也互换。
  - `4115-4118` 右缸正值写入 `right_YG_Bwd`、负值写入 `right_YG_Fwd`；旧版相反，且左缸仍是正向对应 Fwd。可能是右缸安装极性，也可能是映射改错。
  - `2421-2425` 将 `0x185` 角度按无符号 16 位解析；若协议实际为 `int16`，负角会变为约 `655.xx°`。`App_lcd.c:895-896` 又把它加入仰俯角显示，但这不是视频中 Page16 参数成组跳变的直接原因。

- 当前存在但不是 7 月 17 日新引入：
  - `App_lcd.c:327、1862-1864`：`myStr20[10]` 写入“GPS定位成功/失败”需要 12 字节，确定越界 2 字节；链接布局显示会覆盖相邻 `myStr22` 开头。数组至少扩到 12，建议 16，并给字符串函数增加容量参数。

这些问题都应修，但视频中 Page16 整组参数随刷新变化，直接证据仍指向已确认的栈溢出覆盖 `gPwm_BD[3]`。
```

### assistant 2026-07-17T06:56:42.606Z
...
<oai-mem-citation>
<citation_entries>
MEMORY.md:963-986|note=[prior Arm200A helper context was checked during comparison]
</citation_entries>
<rollout_ids>
019ef002-c37e-79d1-8d8e-df6a6f85457c
</rollout_ids>
</oai-mem-citation>
```
