...
- Local source: `C:\Users\t250c\.codex\sessions\2026\07\17\rollout-2026-07-17T15-12-12-019f6eeb-1560-7970-85ae-7a2c77ebcf16.jsonl`

## Transcript

### developer 2026-07-17T07:12:17.912Z
...
### assistant 2026-07-17T07:12:17.914Z
...
### assistant 2026-07-17T07:12:17.918Z
...
### assistant 2026-07-17T07:12:17.921Z
...
### user 2026-07-17T07:12:17.922Z

```text
第16页 相当于上面的数值在跳变。不应该的。
```

### assistant 2026-07-17T07:12:17.922Z
...
### assistant 2026-07-17T07:12:17.923Z
...
### assistant 2026-07-17T07:12:17.925Z
...
### user 2026-07-17T07:12:17.925Z

```text
除了这些，还有没有别的？
```

### assistant 2026-07-17T07:12:17.926Z
...
### assistant 2026-07-17T07:12:17.930Z
...
### developer 2026-07-17T07:12:17.941Z
...
### user 2026-07-17T07:12:17.941Z

```text
好。把它全部修好。
```

### assistant 2026-07-17T07:12:17.948Z
...
### developer 2026-07-17T07:12:17.949Z
...
### assistant 2026-07-17T07:12:24.944Z

```text
我先按 Keil 嵌入式代码和 ViewTech 变量映射两套规则做只读核对，重点限定在第16页最小改法、EEPROM入口和对其他页面的影响；这一步不改文件。由于工程在 E 盘工作区外，也会按全盘访问流程只读检查。
```

### assistant 2026-07-17T07:12:54.055Z

```text
目前已确认第16页有两个独立的界面逻辑错误：反向选择会短暂进入光标0，前四个单数字段却按三位参数的`0～995`规则编辑。我继续把这四位在控制算法中的组合方式和EEPROM默认/读回路径对齐，避免把“显示修复”做成参数含义变化。
```

### assistant 2026-07-17T07:19:05.179Z

```text
我再把 EEPROM 底层读函数的所有调用点和返回值依赖核一遍，确认把 `<= num` 改成 `< num` 不会引入连带问题，然后给出最终最小修复边界。
```

### assistant 2026-07-17T07:27:20.884Z
...
- 第16页参数映射正确：`0x80~0x89` 对应 `gPwm_BD[3]` 的10个参数。
- 跳变主因是移动光标时触发10个数值逐项重发；首4位还错误允许到995。
- 另确认一个更严重的 EEPROM/I2C 内存越界，必须一起修。
...
2. `App_lcd.c:1492-1566` `Par_set16`

   - 用 `initframe` 判断真正进入页面，规范化 `XZ` 到 `1..10`，强制重画光标。
   - 左移由 `1→10`，右移由 `10→1`，禁止出现 `XZ=0` 和地址 `0xAF`。
   - 先处理导航，再画光标。
   - 移动光标不再设置 `key_Fresh`。
   - 加减只在按键上升沿修改，并只写当前选中地址。
   - 默认恢复才刷新整组。
   - 光标只清 `0xB0~0xB9`，不是16个地址。

3. 建议增加 DGUS 连续 word 写函数

   - 第16页 `0x80~0x89` 和光标 `0xB0~0xB9` 都是连续10个 word。
   - 一帧写10个word为26字节，可替代原来的10或17个独立帧。
   - 不能直接复用 `LCD_WR_Str`，它会追加 `FFFF` 并覆盖下一地址。
   - 本地源码证明协议支持连续写，但屏幕内部是否绝对“同时渲染”仍需实机录像确认。

4. EEPROM/I2C 必修

   - `i2c.h:14`：`IICBUFSIZE` 只有128。
   - 当前读取的 `mPwm_BD=132B`、`mBDParam=164B`，ISR已经越界覆盖 `gIICStat/gLCDSets`。
   - `App_Bus.c:151` 又使用 `i<=num`，调用者再多写1字节。
   - 最小安全修复：

     - `IICBUFSIZE` 改为 `0x100`。
     - `I2C_Read_N` 增加 `num==0 || num>IICBUFSIZE` 防护。
     - 复制循环改为 `i<num`。

   当前 RAM 余量充足，增加256字节全局缓冲安全。

5. EEPROM校验

   - `Sys_Read_PwmBD` 每条记录/每次重试前必须重置 `Flg2=0`。
   - 同型问题还存在于 `Sys_Read_Info`、`Sys_Read_BD`、`Sys_Read_BD2`。
   - 第16页参数应在有效读取后及保存前做范围校验；非法值恢复各自默认值，不建议把 `65535` 简单夹到最大值。
   - `I2CEngine` 当前还会对NACK/超时返回成功，这是更大范围的I2C可靠性问题，完整修复应区分正常完成与NACK。

其他同类问题：

- 第8、9、10页也存在反向选择进入 `XZ=0`。
- 第8、10页移动/编辑会引发整组刷新。
- 步长为2或5的参数只判断 `<995`，可能增加到996~999；部分减法还可能越过下限或无符号下溢。

验证重点：

- 第16页首4位只能0~9，后6项只能2~995。
- 1向左直接到10，10向右直接到1，绝不访问`0xAF`。
- 移动光标时不发送`0x80~0x89`数据。
- 保存、断电重启后数值不跳。
- 重新编译检查I2C缓冲区尺寸和RAM占用。
- 实机复拍用户视频中的操作过程。
```
